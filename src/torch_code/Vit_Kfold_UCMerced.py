...
def main():
    ...
    train_data_loader, test_data_loader = get_data(train_batch_size=16, test_batch_size=16)
    ...
    for epoch in trange(TORCH_NUM_EPOCHS):
    ...
    with torch.no_grad():
        print("---" * 30 + f"\nRunning Eval")
        for idx, (images, lb) in tqdm(enumerate(test_data_loader), total=len(test_data_loader)):
